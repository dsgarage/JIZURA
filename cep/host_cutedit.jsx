...
(function () {
    ...
    function rebuildStart(C, s, oldId, opts) {
        ...
        var au = C.cutEdit.audioOf(old), job = null;
        app.beginUndoGroup('JIZURA カット編集: 作り直し');
        try { job = C.start(plan, { roles: roles(C), audioItem: au ? au.item : null, audioStart: au ? au.start : 0 }); }
        finally { app.endUndoGroup(); }
        rb = { job: job, plan: plan, old: old, oldName: old.name, opts: opts || {}, t0: new Date().getTime(), audio: !!au };
